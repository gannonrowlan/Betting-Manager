...
test.describe('Bet Flows', () => {
  ...
  test('edit and delete flows update the saved bet list correctly', async ({ page }) => {
    await registerAndLandOnDashboard(page, {
      name: 'Edit Tester',
    ...
    await page.locator('input[name="stake"]').fill('22');
    await page.locator('select[name="result"]').selectOption('win');
    await page.locator('input[name="betDate"]').fill('2026-03-22');
    await page.getByRole('button', { name: 'Save Bet' }).click();
    await expect(page).toHaveURL(/\/bets\/history$/);
    ...
    await page.locator('input[data-history-checkbox]').first().check();
    page.once('dialog', (dialog) => dialog.accept());
    await page.getByRole('button', { name: 'Delete Selected' }).click();
    await expect(page.getByText('1 bet deleted.')).toBeVisible();
